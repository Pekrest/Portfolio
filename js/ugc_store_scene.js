export class UgcStoreScene extends Phaser.Scene {
  constructor() {
    ...
    this.hoveredAssetId = null; // Track the currently hovered asset
  ...
  create(data) {
    ...
    const map = this.make.tilemap({ key: "ugcStoreMap" });
    ...
    console.log("Physics world bounds set to:", map.widthInPixels, "x", map.heightInPixels);
        ...
        this.physics.add.overlap(this.player, doorSprite, () => {
          ...
          try {
            ...
            if (!mainMap) {
              ...
              const exitPoint = mainMap.findObject("Objects", (obj) => obj.name === "SExit"); // Adjust to match your main map's exit point name
              if (exitPoint) {
                returnX = exitPoint.x;
                returnY = exitPoint.y;
              } 
      ...
      // Make worldLayer interactive for clickable tiles
      worldLayer.setInteractive();
      this.input.setDefaultCursor('default');

      // Hover handler: Tint all tiles with the same assetId
      this.input.on('pointermove', (pointer) => {
        const tile = worldLayer.getTileAtWorldXY(pointer.worldX, pointer.worldY, true);
        const currentAssetId = tile?.properties?.assetId || null;

        if (currentAssetId !== this.hoveredAssetId) {
          // Clear tint from previous asset
          if (this.hoveredAssetId) {
            const prevTiles = worldLayer.filterTiles(t => t.properties.assetId === this.hoveredAssetId);
            prevTiles.forEach(t => t.tint = 0xffffff);
          }

          // Apply tint to new asset
          if (tile && tile.properties.clickable && currentAssetId) {
            const assetTiles = worldLayer.filterTiles(t => t.properties.assetId === currentAssetId);
            assetTiles.forEach(t => t.tint = 0xffff00);
            this.input.setDefaultCursor('pointer');
          } else {
            this.input.setDefaultCursor('default');
          }

          this.hoveredAssetId = currentAssetId;
        }
      });

      // Click handler: Act on any tile of the asset
      this.input.on('pointerdown', (pointer) => {
        const tile = worldLayer.getTileAtWorldXY(pointer.worldX, pointer.worldY, true);
        if (tile && tile.properties.clickable && tile.properties.assetId) {
          const action = tile.properties.action || 'openLink';
          const url = tile.properties.url;
          const startOrderScene = tile.properties.startOrderScene || false;
        if (startOrderScene) {
          this.scene.run('OrderScene', { assetId: tile.properties.assetId });
  ...
  // Set smaller viewport for UgcStoreScene (840x600, centered)
 ...
 // Create life bar (background and fill)
  ...
  this.profileImageBorder = this.add.image(5, 5, 'profileImageBorder')
   ...
   this.profileImage = this.add.image(10, 7, 'profile')
    ...
    this.twitterLink = this.add.sprite(785, 4, 'xButtonDw')
      .setScrollFactor(0)
      .setDepth(34)
    ...
    if (!this.bgMusic || !this.bgMusic.isPlaying) {
      this.bgMusic = this.sound.add("uss_Music", { volume: this.volume, loop: true });
      this.bgMusic.play();
    } else {
      this.bgMusic.setVolume(this.volume); // Ensure volume is updated if returning
    }
   // Add settings button
    this.settingsButton = this.add.sprite(785, 34, 'settingButtonDw')
      .setScrollFactor(0)
      .setDepth(33)
      .setOrigin(1, 0) // Align right edge
      .setInteractive({ useHandCursor: true })
   ...
   this.settingsPanel = this.add.container(730, 103);
